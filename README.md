...
2. [DAG Definition (TaskFlow API)](#dag-definition-taskflow-api)
3. [Task Types & Operators](#task-types--operators)
4. [Task Dependencies & Control Flow](#task-dependencies--control-flow)
5. [XCom & Data Passing](#xcom--data-passing)
6. [Connections & Variables](#connections--variables)
7. [Sensors & Triggers](#sensors--triggers)
8. [Error Handling & Retries](#error-handling--retries)
9. [Scheduling & Triggers](#scheduling--triggers)
...
@dag(
    ...
    default_args=default_args,
    description='Advanced pipeline with all configurations',
    ...
    tags=['advanced', 'production', 'data-pipeline']
)
...
### Python Task (TaskFlow)
```python
@task
def process_data(data_source: str, **context):
    """Process data from specified source"""
...
@task.python
def alternative_python_task():
    """Alternative Python task definition"""
...
### Basic Dependencies
```python
@dag(dag_id='dependency_example', schedule='@daily', start_date=datetime(2024, 1, 1))
def dependency_pipeline():
    ...
    a >> [b, c]  # task_a runs before both task_b and task_c
    [a, b] >> c  # task_c runs after both task_a and task_b
```
...
def dynamic_pipeline():
    ...
    # Map task across multiple inputs
...
def process_large_dataset():
    """Handle large datasets with external storage"""
    import boto3
...
@task
def consume_large_dataset(data_reference):
...
@task
def database_task():
    """Connect to database using Airflow connection"""
    from airflow.hooks.postgres_hook import PostgresHook
...
@task
def api_task():
    """Connect to API using HTTP connection"""
    ...
    response = http_hook.run(
        ...
        headers={'Authorization': 'Bearer {{ conn.api_server.password }}'}
    )
...
3. **XCom data too large**
   - Use external storage for large data
   - Implement custom XCom backend
...
This comprehensive cheat sheet covers all essential aspects of Apache Airflow development using the modern TaskFlow API. It includes practical examples, best practices, and advanced configurations that will help you build robust, scalable data pipelines.
